...
2. 找到Kp后台（如果没有准备TeamViewer)
3. 查看新机器的SN，检查商家是否已经Batch,然后在Kp的后台检查（Check Batch）。Check Batch大部分数字是0，则代表已经过账成功，具体情况具体分析。
4. 通知Harris或其他有激活权限的人，确认Pax系列刷卡机的升级激活权限。
5. 确认激活之后，让商家将刷卡机重启。（重新插拔电源即可）
6. 重启之后，一般来说IP会跳转到新的IP地址。这边先固定IP，然后查看IP地址，再Ping一下，确认有网络之后。查看刷卡机的MID。如果没有网络，再次查刷卡机的ECR，然后再试着Ping。
